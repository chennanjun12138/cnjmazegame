...
class cnjmazegame : AppCompatActivity() {
   public var widths = 0
   public var gamewideh = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
      //view.background = context.getDrawable(R.color.white)
       ...
       // mediaPlayer.start()
        ...
        var frameLayout = findViewById<View>(R.id.frameLayout) as FrameLayout
        var myView =cnjMyView(this)
        myView.background=getDrawable(R.mipmap.keji)
        ...
        view_timer.base = SystemClock.elapsedRealtime() + 60000
        view_timer.start()
        view_timer.setOnChronometerTickListener(OnChronometerTickListener {
        ...
        Timer().schedule(object : TimerTask() {
            override fun run() {
                //if (SystemClock.elapsedRealtime()-view_timer.getBase()>=0)view_timer.stop();
                Looper.prepare()
                val builder = AlertDialog.Builder(this@cnjmazegame)
                builder.setTitle("游戏结束")
                builder.setMessage("遗憾，闯关失败!")
                builder.setPositiveButton("确定"){
                        DialogInterface,which->
                    val intent2 = Intent(this@cnjmazegame, cnjgameset::class.java)
       ...
       b1.setOnClickListener {
           ...
           {     mediaPlayer.stop()
               ...
               val ans=jifeng.toString().toInt()+10
